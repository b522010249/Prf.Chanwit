...
class StudentRepository:
    ...
    def delete(self, student_id: str) -> int:
        with self.db.get_connection() as conn:
            ...
            conn.commit()
            return cur.rowcount
    # เพิ่มเมธอดนี้ใน StudentRepository
    def get_summary_report(self) -> list[tuple[str, str, int]]:
        with self.db.get_connection() as conn:
            # Query จัดกลุ่มตามคณะและสาขา พร้อมนับจำนวน
            rows = conn.execute("""
                SELECT faculty, major, COUNT(student_id) as total
                FROM students
                GROUP BY faculty, major
                ORDER BY faculty, major
            """).fetchall()
            return [(row["faculty"], row["major"], row["total"]) for row in rows]